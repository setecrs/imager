...
function checkMat (device, doc) {
  fs.stat(doc.path, (err, stats) => {
    ...
    if (size === Number(device.properties.udev.SIZE)) {
    ...
    fs.access(mydir + '/hashlog.md5', fs.F_OK, err => {
      if (!err) {
        ...
        exec(`cat "${mydir}"/hashlog.md5 | sed -e '/^#/d' | wc -l`)
        .then(stdout => {
        ...
        .catch(err => {
          console.log('x number of lines on hashlog.md5: ', err.message)
        })
      } else {
        console.log('x hashlog.md5 does not exist ', err.message)
      }
    })
    ...
    fs.access(mydir + '/SARD/IPED.log', fs.F_OK, err => {
      if (!err) {
        ...
        .catch(err => {
          console.log('x IPED: ', err.message)
        })
      } else {
        console.log('x SARD/IPED.log does not exist ', err.message)
      }
    })
    ...
    .catch(err => {
      console.log('x ddrescue not complete: ', err.message)
    })
  })

  if (device.properties.udev.ID_PART_TABLE_UUID) {
    exec(`blkid ${doc.path} | grep 'PTUUID="${device.properties.udev.ID_PART_TABLE_UUID}"' | wc -l`)
    .then((stdout) => {
    ...
    .catch(err => {
      console.log('x partition table UUID on image: ', err.message)
    })
  }

  if (device.properties.udev.ID_FS_UUID) {
    exec(`blkid ${doc.path} | grep 'UUID="${device.properties.udev.ID_FS_UUID}"' | wc -l`)
    .then((stdout) => {
      expect(stdout).equal('1\n')
    })
    .then(() => {
    ...
    .catch(err => {
      console.log('x filesystem UUID on image: ', err.message)
    })
  }
}
